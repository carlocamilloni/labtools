...
static void calc_mat(int nres, int natoms, int rndx[],
                     rvec x[], int *index, int resind[], 
                     real trunc, real cdist, real **mdmat, int **nmat, double **tmat, double ww, int ePBC, matrix box)
{
    int   i, j, resi, resj;
...
static void tot_nmat(int nres, int natoms, double nframes, int **nmat,
...
int gmx_mdmat(int argc, char *argv[])
{
    ...
    t_pargs         pa[]     = {
        { "-t",   FALSE, etREAL, {&truncate},
          "trunc distance" },
        { "-cdist",   FALSE, etREAL, {&cdist},
          "contact distance" },
        { "-natfrac",   FALSE, etREAL, {&frac},
          "contact populations to be considered native" },
...
#define NFILE asize(fnm)
    ...
    do
    {
        ...
        nframes+=ww;
        calc_mat(nres, natoms, rndx, x, index, resind, truncate, cdist, mdmat, nmat, tmat, ww, ePBC, box);
        for (i = 0; (i < nres); i++)
        {
            for (j = 0; (j < natoms); j++)
            {
